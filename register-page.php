...
<meta charset=utf-8><!--for escaping problem characters-->
...
<div id="content"><!-- Registration handler content starts  -->
<p>
<?php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	//Check for age
		if (empty($_POST['age'])) {
			...
			$a = mysqli_escape_string($dbcon,trim($_POST['age']));
		}
	//Check for gender
	if (empty($_POST['gender'])) {
		$errors[] = 'You forgot to select your gender.';
	} else {
		$g = mysqli_escape_string($dbcon,trim($_POST['gender']));
	}
...
// Make the query:
$q = "INSERT INTO users ( fname, lname,age,gender,userName, email, psword, registration_date)
VALUES ('$fn', '$ln','$a','$g','$u', '$e', SHA1('$p'), NOW() )";
...
<form action="register-page.php" method="post">
	<p><label class="label" for="fname">First Name:</label><input id="fname" type="text" name="fname" size="30" maxlength="30" value="<?php if (isset($_POST['fname'])) echo $_POST['fname']; ?>"></p>
		...
		<p><label class ="label" for ="age">Age:</label><input id="age" type="text" name="age" size="4" maxlength="4" value ="<?php if(isset($_POST['age'])) echo $_POST['age']; ?>"></p>
		<p><label class ="label" for ="gender">Gender:</label><input name="gender" type="radio" id="male" value="M" >Male<input name="gender" type="radio" id="female" value="F" >Female</p>
		<p><label class="label" for="user">Username:</label><input id="user" type="text" name="user" size="30" maxlength="40" value="<?php if (isset($_POST['user'])) echo $_POST['user']; ?>"></p>
